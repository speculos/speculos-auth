...
function parseOrder(p_context, p_fields, p_default) {
	let orders = p_context.validate(p_context.request.query.order, VALIDATOR_ORDER.optional().default(p_default));
	if(!orders) {
		return [];
	}
...
function attach() {
	return function*(p_next) {
		...
		this.pagination = {
			...
			parseOrder:function(p_fields) { return parseOrder(self, p_fields); }
		};
